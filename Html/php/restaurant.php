...
$stmt = $conn->prepare("SELECT RName,Area,AveragePrice,Foodtype FROM Restaurant JOIN Foodinfo WHERE RName LIKE '%?%' AND Area LIKE '%?%'");
$stmt->bind_param("", $RName,$Area);
if(isset($_POST["Restaurant NAME"])and !empty($_POST["Restaurant NAME"])){
    $AName=$_POST["Restaurant NAME"];
}
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "Name: " .$row["RName"] ." - Type: ". $row["Foodtype"]." -Area: " .$row["Area"]." Average price: ". $row["AveragePrice"]."<br>";
    }
} else {
